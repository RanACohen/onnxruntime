...
OpKernelContext::OpKernelContext(IExecutionFrame& frame, const OpKernel& kernel, const logging::Logger& logger,
                                 concurrency::ThreadPool* thread_pool)
    : execution_frame_(frame), kernel_(kernel), logger_(&logger), thread_pool_(thread_pool) {
...
int OpKernelContext::GetInputArgIndex(int index) const { return node_input_start_index_ + index; }

int OpKernelContext::GetImplicitInputArgIndex(int index) const { return node_implicit_input_start_index_ + index; }

int OpKernelContext::GetOutputArgIndex(int index) const { return node_output_start_index_ + index; }
